...
//    public String create(ArticleCreateRequest req){  //폼에서 입력한 정보가 req객체로 넘어오는거지
...
//    }
    ...
    public String editForm(@PathVariable Long id, Model model){  // 스프링이 모델 제공!! 모델에 담아서 이동한다는거지
        var article = articleService.findById(id); // Article article = articleService.findById(id);
        model.addAttribute("article", article);
        return "article-edit";
    }
